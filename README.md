...
## Unexpected Problems
- The provided starter repository contained only the default JavaFX "Hello World" Maven archetype (`HelloApplication`, `HelloController`, `Launcher`) with no actual 2048 implementation underneath it, despite the coursework brief describing this as a "maintenance" exercise on an existing game. The full game (model, controller, menu, persistence, tests) was designed and implemented from that bare scaffold rather than refactored from pre-existing game logic.
